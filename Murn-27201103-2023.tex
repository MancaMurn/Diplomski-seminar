...
\begin{lema} \label{lema:1}
    ...
    \begin{enumerate}
        ...
        \item $\forall x, y \in H_i(a) \; \forall z \in H_j(a): i \neq j \Rightarrow d_{G[H]}(x,z) = d_{G[H]}(y,z).$
    \end{enumerate}
\end{lema}
...
\begin{dokaz}
    ...
    \begin{enumerate}
        ...
        \item Označimo $x = (a, u_i), \; y = (a, u_j), \; z = (a, u_k)$ za neke 
        $u_i, u_j \in V(H_i), \; u_k \in V(H_j)$.
        Po \eqref{eq:razdalja_produkta} sledi $d_{G[H]}((a, u_i), (a, u_k)) = 
...
\begin{dokaz}
    ...
    $V(H_i) \geq 2$ in velja $ W_i(a) = \emptyset$. Potem obstajata vsaj dve vozlišči iz $H_i(a)$ z različnimi
    metričnimi predstavitvami glede na $W$, za katerega velja $W \subseteq \cup $ kar je v protislovju s \ref{lema:1}, saj je $H_i(a) \subseteq H(a)$.
...
\begin{dokaz}
    ...
    Denimo, da $W(a) \cap S < m-1$. Potem obstajata vsaj dve vozlišči $(a, v_i), (a, v_j) \in S,$ za kateri velja
    $d((a, v_i), (a, w)) \neq d((a, v_i), (a, w)) \; \forall (a, w) \in W(a) \subseteq W.$  
    Ker je $W(a) \subseteq H(a),$ je to v protislovju s \ref{lema:1}.
\end{dokaz}
...
\begin{lema} \label{lema:2}
    ...
    $W \subseteq V(Q).$
\end{lema}
% Preglej ta dokaz, malo ga ne razumem.
\begin{dokaz}
...
\begin{dokaz}
    ...
    Velja $d((a,x),(a,z)) = d((a,y),(a,z))$ za vsak $(a,z) \in W(a).$ 
\end{dokaz}
...
% We consider two different vertices u, v ∈ V(G). Let PG(u, v) be a shortest u−v path in G and ℓ(PG(u, v)) be the length of
% PG(u, v). We define PG(u, v) = {PG(u, v)}. Let z be a vertex in V(G) \ V(PG(u, v)). If ℓ(PG(u, v))+ℓ(PG(v, z)) > ℓ(PG(u, z))
% and ℓ(PG(u, v)) + ℓ(PG(u, z)) > ℓ(PG(v, z)), then each path in PG(u, v) is called an eccentric path of G.
