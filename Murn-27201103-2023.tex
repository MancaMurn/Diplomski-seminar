...
V tem diplomskem seminarju si bomo ogledali pojema metrične in sosedske dimenzije grafa in njune 
osnovne lastnosti, ter povezave med njima, definirali bomo leksikografski produkt grafov ter povzeli 
...
\begin{definicija}
    Naj bo $G$ poljuben graf in $W = \{ w_1, ... , w_k  \} \subseteq V(G)$. Podmnožica
    vozlišč. Vektorju $s_W(v) = (a(v, w_1), ..., a(v, w_k))$, kjer je $a$ rečemo 
    sosedska predstavitev vozlišča $v \in V(G)$ s podmnožico $W$.
\end{definicija}

\begin{definicija}
    Podmnožica vozlišč $S \subseteq V(G)$ je sosedsko rešljiva,
    če $\forall u, v \in V(G): u\neq v \implies s_S(v) \neq s_S(u)$.
\end{definicija}

\begin{definicija}
    Najmanjša sosedsko rešljiva množica grafa $G$ se imenuje sosedska baza. Njeno velikost 
    imenujemo sosedska dimenzija in jo označimo z $\mu (G).$
\end{definicija}
...
vezli tudi vozlišče $v_n$ in prišli do enakega rezultata. 
