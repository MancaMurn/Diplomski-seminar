...
%TA PODNASLOV NI NAJBOLJ POSREČEN\ldots
V tem razdelku obravnavamo leksikografski produkt $G[H]$, kjer je $G$ povezan graf reda vsaj $2$ 
in $H$ poljuben graf reda vsaj $2$. Naj bosta $a \in V(G)$ in $b \in V(H)$ poljubni vozlišči. 
...
\begin{lema}
    Naj bosta $a, b \in V(G)$ različni vozlišči. Potem velja 
    \begin{enumerate}
        \item $\forall x, y \in H(a), z \in H(b): d_{G[H]}(x,z) = d_{G[H]}(y,z).$
        \item $\forall x, y \in H_i(a), z \in H_j(a): d_{G[H]}(x,z) = d_{G[H]}(y,z).$
    \end{enumerate}
\end{lema}
\begin{proof}
    \begin{enumerate}
        \item Velja $x = (a, u_i), y = (a, u_j), z = (b, u_k)$ za neke $u_i, u_j, u_k \in V(H)$.
        Po formuli \eqref{eq:razdalja_produkta} velja $d_{G[H]}((a, u_i), (b, u_k))
        = d_G(a, b) = d_{G[H]}((a, u_j), (b, u_k))$, saj sta $a$ in $b$ različni vozlišči.
        \item Velja $x = (a, u_i), y = (a, u_j), z = (a, u_k)$ za neke $u_i, u_j \in V(H_i), u_k \in V(H_j)$.
        Po formuli \eqref{eq:razdalja_produkta} velja $d_{G[H]}((a, u_i), (a, u_k)) = 
        a_H(u_i, u_k) = d_{G[H]}((a, u_j), (a, u_k))$, saj sta $a$ in $b$ različni vozlišči.
    \end{enumerate}
 \end{proof}
...
\begin{lema}
    Naj bo $W$ metrična baza grafa $G[H]$, kjer je $G$ povezan graf reda vsaj $2$ in $H$ graf reda vsaj $2$, ki vsebuje 
    $k$ komponent. Potem velja
   $$\forall a \in V(G) \; \forall i \in \{1, 2, \ldots, k\} : V(H_i) \geq 2 \Rightarrow W \cap H_i(a) \neq \emptyset.$$
   Velja še več, če je $B_i$ metrična baza $H_i$, potem je $|W \cap H_i(a)| \geq \beta(H_i).$
\end{lema}

\begin{dokaz}
    Denimo, da obstaja $a \in V(G)$ za kateterega obstaja tak $i \in \{1, 2, \ldots, k\}$, da je
    $V(H_i) \geq 2$ in velja $ W \cap H_i(a) = \emptyset$. Potem obstajata vsaj dve vozlišči iz $H_i(a)$, ki imata različni
    metrični predstavitve glede na $W$. 
